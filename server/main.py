...
async def chat(req: ChatRequest):
    ...
    if mission_state:
        flags = []
        if mission_state.get("powerReduced"): flags.append("Power reduced to 12 amps ✓")
        if mission_state.get("co2Fixed"): flags.append("CO2 scrubber fix implemented ✓")
        if mission_state.get("burnExecuted"): flags.append("PC+2 burn executed ✓")
        if mission_state.get("emergencyDeclared"): flags.append("Emergency declared ✓")
        if mission_state.get("lemActivated"): flags.append("LEM Aquarius activated as lifeboat ✓")
        if mission_state.get("navigationTransferred"): flags.append("Navigation transferred to LEM ✓")
        if mission_state.get("freeReturn"): flags.append("Free-return trajectory confirmed ✓")
        directives = mission_state.get("pendingDirectives", [])
        if directives:
            flags.append(f"Pending directive: {directives[-1]['text']}")
        if flags:
...
- If shared context shows what other stations said, you may reference it naturally.{transcript_text}{timeline_text}{shared_ctx_text}{state_text}"""
